...
describe('CSVImport', () => {
  it('renders the drop zone with instructions', () => {
    ...
    expect(screen.getByText('Drag & drop your bank CSV here')).toBeInTheDocument();
  ...
  it('handles drag enter/over/leave cycle correctly', () => {
    render(<CSVImport onImport={vi.fn()} transactionCount={0} />);
    const dropZone = screen.getByText('Drag & drop your bank CSV here').closest('div')!;
  ...
  it('shows error for non-CSV files on drop', async () => {
    render(<CSVImport onImport={vi.fn()} transactionCount={0} />);
    const dropZone = screen.getByText('Drag & drop your bank CSV here').closest('div')!;
    ...
    await waitFor(() => {
      expect(screen.getByText(/Please drop a CSV file/)).toBeInTheDocument();
    });
  });
